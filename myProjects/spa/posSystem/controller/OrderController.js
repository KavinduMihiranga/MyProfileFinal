...
function displayQty() {
    var removeItem=$('#txtOrderQuantity').val();
    var qtyResponse=searchRemoveItemQ(removeItem);
    if (qtyResponse){
        var qty=qtyResponse.oQuantityOnHand-qtyResponse.oQuantity
        $('#txtQuantityOnHand').val(qty)
    }
}
function searchRemoveItemQ(code){
...
function checkQty() {
    ...
    if (qty>qtyOnHand){
        ...
        alert('Quantity Ok')
        $('#txtQuantityOnHand').val(qtyOnHand-qty);
    }
}
